<?php
    ...
    $executionStatusByAgent = $case->agentExecutions
        ...
        ->all();
?>
<!-- Agent Timeline Live Component - Last Updated: <?php echo e(now()->format('Y-m-d H:i:s')); ?> -->
<style>
...
<div class="bg-white p-6 rounded-xl border border-primary/10 shadow-sm" id="agentTimeline">
    <div class="flex justify-between items-center mb-4">
        <div class="flex items-center gap-3">
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(in_array($statusVal, ['phase1_processing', 'phase2_processing', 'phase3_processing'], true)): ?>
                <button
        ...
        <div class="flex items-center gap-4">
            <span class="text-sm font-bold text-slate-600">
                <span id="currentStep">1</span> من <span id="totalSteps"><?php echo e(count($definitions)); ?></span>
            </span>
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(in_array($statusVal, ['failed', 'paused'], true)): ?>
                <button 
                    onclick="retryCase()"
    ...
    <div class="space-y-3" id="agentsContainer">
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $definitions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $def): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
            <div class="agent-card border rounded-xl status-transition border-slate-200 overflow-hidden" id="agent-container-<?php echo e($def['number']); ?>" data-agent="<?php echo e($def['number']); ?>">
                
                <div
                    role="button"
                    tabindex="0"
                    onclick="toggleAgent(<?php echo e($def['number']); ?>)"
                    onkeydown="if(event.key==='Enter')toggleAgent(<?php echo e($def['number']); ?>)"
                    ...
                    id="agent-header-<?php echo e($def['number']); ?>">
                    <div class="flex-shrink-0 size-9 rounded-full flex items-center justify-center bg-slate-300 text-slate-600 transition-all duration-300"
                         id="agent-icon-<?php echo e($def['number']); ?>">
                        <span class="material-symbols-outlined text-lg">schedule</span>
                    </div>
                    <div class="flex-1 min-w-0 text-right">
                        <p class="font-semibold text-sm">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($def['number'] === 0): ?>
                                <span class="text-xs font-bold text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded mr-1">م١</span>
                                <?php echo e($def['name']); ?>

                            <?php elseif($def['phase'] === 2): ?>
                                <span class="text-xs font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded mr-1">م٢</span>
                                <?php echo e($def['number']); ?>. <?php echo e($def['name']); ?> — <?php echo e($def['name_en']); ?>

                            <?php else: ?>
                                <span class="text-xs font-bold text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded mr-1">م٣</span>
                                <?php echo e($def['name']); ?> — <?php echo e($def['name_en']); ?>

                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </p>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!empty($def['outputs'])): ?>
                            <p class="text-xs text-slate-500 mt-0.5"><?php echo e(implode(', ', $def['outputs'])); ?></p>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    </div>
                    <div class="flex-shrink-0 flex items-center gap-2">
                        <button type="button"
                            onclick="event.stopPropagation(); retryFromAgent(<?php echo e($def['number']); ?>)"
                            ...
                            id="agent-rerun-<?php echo e($def['number']); ?>"
                        ...
                        <button type="button"
                            onclick="event.stopPropagation(); openModelConfigForAgent(<?php echo e($def['number']); ?>)"
                            ...
                            id="agent-model-btn-<?php echo e($def['number']); ?>"
                        ...
                        <button type="button"
                            onclick="event.stopPropagation(); openSystemMessageEditor(<?php echo e($def['number']); ?>)"
                            ...
                            id="agent-sysmsg-btn-<?php echo e($def['number']); ?>"
                            title="تعديل رسالة النظام">
                            <span class="material-symbols-outlined text-sm">edit_note</span>
                            رسالة
                        </button>
                        <div class="text-xs font-bold" id="agent-status-<?php echo e($def['number']); ?>">
                            <span class="text-slate-400">في الانتظار</span>
                        </div>
                        <span class="material-symbols-outlined text-slate-400 text-base transition-transform duration-200" id="agent-expand-<?php echo e($def['number']); ?>" style="display:none;">expand_more</span>
                    </div>
                </div>

                
                <div id="agent-output-<?php echo e($def['number']); ?>" class="hidden border-t border-slate-200 agent-expand-transition">
                    <div class="p-5 bg-gradient-to-b from-slate-50 to-white">
                        <h4 class="text-xs font-bold text-slate-700 uppercase flex items-center gap-2 mb-3 status-transition" id="agent-output-header-<?php echo e($def['number']); ?>">
                            <span class="material-symbols-outlined text-sm text-primary">terminal</span>
                            مخرجات الوكيل (مباشر)
                        </h4>

                        
                        <div id="agent-accuracy-warning-<?php echo e($def['number']); ?>"
                             class="mb-3 flex items-start gap-3 bg-amber-50 border border-amber-300 rounded-xl px-4 py-3 text-sm text-amber-800 <?php echo e(isset($exhaustedAgents[$def['number']]) ? '' : 'hidden'); ?>">
                        ...
                        <div class="bg-white rounded-lg border border-slate-200 p-4 max-h-[500px] overflow-y-auto agent-output-content shadow-inner"
                             id="agent-content-<?php echo e($def['number']); ?>"
                             style="direction: rtl; white-space: pre-wrap; font-family: 'Cairo', system-ui; font-size: 14px; line-height: 1.9;">
                            <span class="text-slate-400 text-sm animate-pulse" id="agent-placeholder-<?php echo e($def['number']); ?>">جارٍ الانتظار...</span>
                            <span class="streaming-output" id="agent-stream-<?php echo e($def['number']); ?>"></span>
                        </div>
                    </div>
                </div>
            </div>
...
let currentCaseStatus = '<?php echo e($case->status->value ?? $case->status); ?>'; // Updated dynamically from SSE

// DB outputs keyed by agent_number for pre-population on refresh
const dbOutputsByAgent = <?php echo json_encode($outputsByAgent, 15, 512) ?>;
const currentAgent = <?php echo e($case->current_agent ?? 'null'); ?>;
// Agent definitions keyed by number (for name lookups in JS)
const agentDefinitions = <?php echo json_encode(collect($definitions)->keyBy('number')->toArray(), 15, 512) ?>;
// Agent numbers whose self-correction was exhausted (for page-load state)
const dbExhaustedAgents = <?php echo json_encode(array_keys($exhaustedAgents), 15, 512) ?>;
// Execution status by agent_number — fallback for agents with no markdown outputs
const dbExecutionStatuses = <?php echo json_encode($executionStatusByAgent, 15, 512) ?>;
...
function startSSE() {
    ...
    const caseStatus = '<?php echo e($case->status->value ?? $case->status); ?>';
    ...
    const sseUrl = `/cases/<?php echo e($case->id); ?>/stream`;
...
function updateProgress() {
    const total = <?php echo e(count($definitions)); ?>;
    let completed = 0;
    let inProgress = 0;
...
function stopProcessing() {
    ...
    fetch('/cases/<?php echo e($case->id); ?>/abort', {
        method: 'POST',
        headers: {
...
function retryCase() {
    ...
    form.method = 'POST';
    form.action = '<?php echo e(route("cases.retry-agent", $case)); ?>';
...
function retryFromAgent(agentNumber) {
    closePausedModal();
    fetch(`/cases/<?php echo e($case->id); ?>/rerun-from`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
...
async function autoLaunchPhase3() {
    ...
    const url = '<?php echo e(route("cases.start-phase3", $case)); ?>';
...
<?php /**PATH /var/www/html/resources/views/components/agent-timeline-live.blade.php ENDPATH**/ ?>