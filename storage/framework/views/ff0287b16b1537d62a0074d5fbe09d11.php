...
<body class="bg-background-light dark:bg-background-dark min-h-screen flex items-center justify-center p-4">
    <div class="flex w-full h-full bg-white dark:bg-slate-900 rounded-xl overflow-hidden shadow-2xl border border-border-gray dark:border-primary/20 max-w-[550px]">
        <div class="w-full flex flex-col justify-center px-8 sm:px-16 py-12 lg:px-16">
            ...
            <form action="<?php echo e(route('login')); ?>" class="space-y-6" method="POST">
                <?php echo csrf_field(); ?>
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input class="w-full pr-12 pl-4 py-3.5 bg-slate-50 dark:bg-slate-800 border border-border-gray dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all placeholder:text-slate-400 <?php $__errorArgs = ['email'];
...
unset($__errorArgs, $__bag); ?>" id="email" name="email" placeholder="example@law.sa" required type="email" value="<?php echo e(old('email')); ?>"/>
                    </div>
                </div>
            ...
            <p class="mt-10 text-center text-slate-600 dark:text-slate-400">
                ليس لديك حساب؟
                <a class="font-bold text-primary hover:text-secondary-green transition-colors mr-1" href="<?php echo e(route('register')); ?>">إنشاء حساب جديد</a>
            </p>
        </div>
    </div>
