<div id="phase2ApprovalModal" 
     ...
     style="display: none;"
     data-passing-threshold="{{ config('legal.audit_passing_threshold', 70) }}"
     data-soft-timeout="{{ config('legal.audit_soft_timeout_seconds', 10) }}"
     data-hard-timeout="{{ config('legal.audit_hard_timeout_seconds', 30) }}"
     data-audit-url="{{ route('cases.audit', $case) }}"
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl max-w-4xl w-full p-8 animate-fade-in max-h-[90vh] overflow-y-auto">
        ...
        <div class="bg-slate-50 rounded-xl p-6 mb-6">
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <p class="text-sm font-medium text-slate-900">{{ $case->title ?? 'غير محدد' }}</p>
                </div>
                <div>
        ...
        <div class="bg-slate-50 rounded-xl p-6 mb-6">
            ...
            @if($case->requiredLaws && $case->requiredLaws->count())
                    ...
                    @foreach($case->requiredLaws as $law)
                        <li class="flex items-start gap-2 text-sm">
                            ...
                            <div>
                                ...
                                @if($law->subject_area)
                                    <span class="text-xs text-slate-500 mr-2">({{ $law->subject_area }})</span>
                                @endif
                                @if($law->reason)
                                    <p class="text-xs text-slate-500 mt-0.5">{{ $law->reason }}</p>
                                @endif
                                @unless($law->law_registry_id)
                ...
                @if($case->requiredLaws->whereNull('law_registry_id')->count() > 0)
        ...
        <div class="mb-6">
            ...
            <div id="agentControlPanel" class="hidden mt-3 border border-slate-200 rounded-xl overflow-hidden">
                ...
                <div class="bg-slate-50 px-4 py-3 border-b border-slate-200 flex items-center justify-between">
                    <span class="text-xs font-bold text-slate-600 uppercase">{{ $allAgentDefs->count() }} وكيل</span>
                    <span class="text-xs text-slate-400">النموذج الافتراضي: {{ $globalModel }}</span>
                </div>

                <!-- Agent Rows -->
                <div class="divide-y divide-slate-100">
                @foreach($allAgentDefs as $def)
                @php
                    $agentNum = $def['number'];
                    $agentPhase = $def['phase'] ?? 2;
                    $currentModel = $caseModelOverrides[$agentNum] ?? null;
                @endphp
                <div class="p-4 hover:bg-slate-50/50 transition-colors" id="agentRow{{ $agentNum }}">
                    <!-- Agent Header Row -->
                    <div class="flex items-center gap-3 mb-3">
                        <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 {{ $agentPhase >= 3 ? 'bg-indigo-100 text-indigo-700' : 'bg-primary/10 text-primary' }} text-xs font-black">
                            {{ $agentNum }}
                        </div>
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2">
                                <span class="text-sm font-bold text-slate-900">{{ $def['name'] }}</span>
                                @if($agentPhase >= 3)
                                    <span class="text-xs bg-indigo-100 text-indigo-600 px-1.5 py-0.5 rounded font-medium">مرحلة ٣</span>
                                @endif
                            </div>
                            <span class="text-xs text-slate-500">{{ $def['name_en'] ?? '' }}</span>
                        </div>
                        <!-- Model Badge -->
                        ...
                        <!-- Expand Button -->
                        <button type="button" onclick="toggleAgentRow({{ $agentNum }})"
                                class="flex-shrink-0 p-1 rounded hover:bg-slate-200 transition-colors">
                            <span class="material-symbols-outlined text-slate-400 text-base transition-transform duration-200" id="agentRowChevron{{ $agentNum }}">tune</span>
                        </button>
                    </div>

                    <!-- Expandable Controls -->
                    <div id="agentRowControls{{ $agentNum }}" class="hidden space-y-3 pl-11">
                        <!-- Model Selector -->
                        <div>
                            <label class="block text-xs font-bold text-slate-600 mb-1.5">النموذج</label>
                            <div class="flex gap-2">
                                <select id="agentModelSelect{{ $agentNum }}"
                                    ...
                                    <option value="">— افتراضي ({{ $globalModel }}) —</option>
                                    @foreach($agentModelGroups as $groupName => $models)
                                        <optgroup label="{{ $groupName }}">
                                            @foreach($models as $modelId => $modelLabel)
                                                <option value="{{ $modelId }}" {{ $currentModel === $modelId ? 'selected' : '' }}>{{ $modelLabel }}</option>
                                            @endforeach
                                        </optgroup>
                                    @endforeach
                                </select>
                                <button type="button" onclick="saveModalAgentModel({{ $agentNum }})"
                                ...
                                @if($currentModel)
                                <button type="button" onclick="resetModalAgentModel({{ $agentNum }})"
                                        class="text-xs px-3 py-2 bg-slate-200 text-slate-600 rounded-lg hover:bg-slate-300 transition-colors">
                                    إعادة تعيين
                                </button>
                                @endif
                            </div>
                            <p id="agentModelStatus{{ $agentNum }}" class="text-xs text-slate-400 mt-1 hidden"></p>
                        </div>
                        ...
                        <div>
                            <div class="flex items-center justify-between mb-1.5">
                                ...
                                <button type="button" onclick="loadAgentSystemMsg({{ $agentNum }})"
                                        class="text-xs text-primary hover:underline flex items-center gap-1">
                                    <span class="material-symbols-outlined text-xs">refresh</span> تحميل
                                </button>
                            </div>
                            <textarea id="agentSysMsgText{{ $agentNum }}"
                                      rows="4"
                            ...
                            <div class="flex items-center justify-between mt-1.5">
                                <span id="agentSysMsgStatus{{ $agentNum }}" class="text-xs text-slate-400"></span>
                                <button type="button" onclick="saveAgentSystemMsg({{ $agentNum }})"
        ...
        <div class="flex gap-3">
            <form id="startPhase2Form" method="POST" action="{{ route('cases.start-phase2', $case) }}" class="flex-1">
                @csrf
            ...
            <a href="{{ route('cases.index') }}" class="px-6 py-4 bg-slate-100 text-slate-500 font-bold rounded-xl hover:bg-slate-200 transition-colors flex items-center justify-center">
                إلغاء
            </a>
        </div>
        ...
        <div id="requestChangesForm" class="hidden mt-6 pt-6 border-t border-slate-200">
            ...
            <form method="POST" action="{{ route('cases.request-changes', $case) }}">
                @csrf
                <div class="space-y-4">
                    <div>
...
let modalCaseStatus = '{{ $case->status->value ?? $case->status }}'; // Current status, updated via SSE (prefixed to avoid conflict with timeline's currentCaseStatus)
...
function handleInlineFile(label, input) {
    ...
    fetch('{{ route("cases.audit-upload", $case) }}', {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': csrfToken
        },
...
document.getElementById('startPhase2Form').addEventListener('submit', function(e) {
    ...
    if (hasInlineText || hasInlineSelections) {
        ...
        fetch('{{ route("cases.update-missing-info", $case) }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken
...
// AGENT CONTROL PANEL
// ============================================
const MODAL_CASE_ID = '{{ $case->id }}';
const MODAL_CSRF = '{{ csrf_token() }}';
