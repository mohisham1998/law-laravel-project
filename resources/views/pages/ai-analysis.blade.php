...
<div x-data="aiAnalysis()" x-init="init()" class="space-y-6">
    ...
    <template x-if="!selectedCase">
        <div class="bg-white border border-dashed border-slate-300 rounded-2xl p-16 text-center">
            ...
            <a href="{{ route('cases.create') }}" class="inline-flex items-center gap-2 bg-primary text-white px-6 py-2.5 rounded-xl font-bold hover:brightness-110 transition-all">
    ...
    {{-- Main content when case selected --}}
    <template x-if="selectedCase">
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                @foreach($agentDefinitions as $agent)
                @php
                ...
                <div
                    ...
                    :class="getAgentCardClass({{ $agent['number'] }})"
                    x-data="{}"
                >
                    <div class="p-4">
                        <div class="flex items-center gap-3 mb-3">
                            <div class="w-9 h-9 rounded-lg flex items-center justify-center transition-colors"
                                 :class="getAgentIconClass({{ $agent['number'] }})">
                                <span class="material-symbols-outlined text-base">{{ $icon }}</span>
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2">
                                    <p class="font-bold text-sm text-slate-900 truncate">{{ $agent['name'] }}</p>
                                    <span class="text-[10px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded-full shrink-0">#{{ $agent['number'] }}</span>
                                </div>
                                <p class="text-[11px] text-slate-400">{{ $agent['name_en'] }}</p>
                            </div>
                            <div class="shrink-0">
                                <template x-if="getAgentStatus({{ $agent['number'] }}) === 'completed'">
                                    <span class="material-symbols-outlined text-emerald-500">check_circle</span>
                                </template>
                                <template x-if="getAgentStatus({{ $agent['number'] }}) === 'running'">
                                    <div class="w-5 h-5 border-2 border-primary border-t-transparent rounded-full animate-spin"></div>
                                </template>
                                <template x-if="getAgentStatus({{ $agent['number'] }}) === 'failed'">
                                    <span class="material-symbols-outlined text-red-500">error</span>
                                </template>
                                <template x-if="getAgentStatus({{ $agent['number'] }}) === 'pending'">
                        ...
                        <div class="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden mb-2">
                            <div class="h-full rounded-full transition-all duration-500"
                                 :class="getAgentProgressColor({{ $agent['number'] }})"
                                 :style="'width: ' + getAgentProgress({{ $agent['number'] }}) + '%'">
                            </div>
                        </div>

                        <div class="flex items-center justify-between">
                            <span class="text-[11px] font-medium" :class="getAgentTextColor({{ $agent['number'] }})" x-text="getAgentLabel({{ $agent['number'] }})"></span>
                            <span class="text-[10px] text-slate-400">{{ $phaseLabel }}</span>
                        </div>

                        {{-- Live output snippet --}}
                        <template x-if="getAgentOutput({{ $agent['number'] }})">
                            <div class="mt-2 bg-slate-50 rounded-lg p-2 border border-slate-100">
                                <p class="text-[11px] text-slate-600 font-mono leading-relaxed line-clamp-2"
                                   x-text="getAgentOutput({{ $agent['number'] }})"></p>
                            </div>
                        </template>

                        {{-- Error message --}}
                        <template x-if="getAgentError({{ $agent['number'] }})">
                            <div class="mt-2 bg-red-50 rounded-lg p-2 border border-red-100">
                                <p class="text-[11px] text-red-600 line-clamp-2" x-text="getAgentError({{ $agent['number'] }})"></p>
                            </div>
                        </template>
                    </div>
                </div>
...
function aiAnalysis() {
    return {
        // ── State ──────────────────────────────────────────────────────
        selectedCase: @json($case),
        allCases: @json($allCases),
        agentDefinitions: @json($agentDefinitions),
