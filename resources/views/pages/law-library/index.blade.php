...
{{-- Search & Filters --}}
<form id="filterForm" method="GET" action="{{ route('law-library.index') }}" class="bg-white rounded-xl border border-slate-200 shadow-sm p-4 mb-4">
    <div class="flex flex-col gap-3">
        ...
        <div class="flex gap-2">
            ...
            <button type="submit"
                    class="px-5 py-2.5 bg-primary text-white rounded-lg text-sm font-bold hover:bg-primary/90 transition-colors cursor-pointer shrink-0 flex items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" class="w-4 h-4"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
                بحث
            </button>
            @if(request()->hasAny(['search', 'category', 'status', 'sort']))
                <a href="{{ route('law-library.index') }}"
                   class="px-4 py-2.5 bg-slate-100 text-slate-500 rounded-lg text-sm font-medium hover:bg-slate-200 transition-colors cursor-pointer shrink-0 flex items-center gap-1.5">
        ...
        <div class="flex flex-wrap gap-2 items-center">
            ...
            <span class="text-xs text-slate-400 font-medium ml-1">فلترة:</span>

            {{-- Category --}}
            <select name="category" onchange="document.getElementById('filterForm').submit()"
                    class="px-3 py-2 rounded-lg border text-sm cursor-pointer transition-colors
                        {{ request('category') ? 'border-primary bg-primary/5 text-primary font-semibold' : 'border-slate-200 bg-white text-slate-600' }}">
            ...
            <select name="status" onchange="document.getElementById('filterForm').submit()"
                    class="px-3 py-2 rounded-lg border text-sm cursor-pointer transition-colors
                        {{ request('status') ? 'border-primary bg-primary/5 text-primary font-semibold' : 'border-slate-200 bg-white text-slate-600' }}">
            ...
            <select name="sort" onchange="document.getElementById('filterForm').submit()"
                    class="px-3 py-2 rounded-lg border text-sm cursor-pointer transition-colors
                        {{ request('sort') && request('sort') !== 'newest' ? 'border-primary bg-primary/5 text-primary font-semibold' : 'border-slate-200 bg-white text-slate-600' }}">
            ...
            {{-- Active filter chips --}}
            @if(request('search'))
                <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-primary/10 text-primary rounded-full text-xs font-semibold">
            ...
            @endif
            @if(request('category'))
                <span class="inline-flex items-center gap-1 px-2.5 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-semibold">{{ request('category') }}</span>
            @endif
            @if(request('status'))
