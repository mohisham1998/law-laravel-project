...
<form action="{{ route('settings.update') }}" method="POST" id="settingsForm" class="min-w-0"
    x-data="settingsForm()"
...
>
    ...
    {{-- Puter token is NOT submitted in form — sent via header at case dispatch time --}}
    ...
    {{-- Two columns: responsive gap; stack on small screens, side-by-side on lg+ --}}
            ...
            <div class="bg-white p-6 sm:p-8 rounded-xl border border-primary/10 shadow-sm lg:sticky lg:top-24 min-h-[28rem] w-full flex flex-col">
                ...
                <div class="space-y-8 flex-1 flex flex-col">
                    <div>
                        ...
                        <select id="modelSelect" class="w-full">
                            @foreach($models as $model)
                                <option value="{{ $model['id'] }}"
                                    ...
                                    data-tier="{{ (isset($model['pricing']['prompt']) && (float)$model['pricing']['prompt'] == 0 && (float)$model['pricing']['completion'] == 0) ? 'free' : 'paid' }}"
                        ...
                        {{-- OpenRouter Sample Output Panel --}}
...
var _settingsInitialProvider = '{{ $llmProvider }}';
var _settingsInitialPuterModel = '{{ $puterModel }}';
var _settingsDisclosureAcknowledged = {{ $puterDisclosureAcknowledged ? 'true' : 'false' }};
var _settingsNotificationsEnabled = {{ $notificationsEnabled ? 'true' : 'false' }};
var _puterModelsUrl = '{{ route('api.puter-models') }}';
...
<script>
// ─── Sample Preview ───────────────────────────────────────────────
var _modelPreviewUrl = '{{ route('api.model-preview') }}';
var _previewXsrf = '{{ csrf_token() }}';
var _previewSeq = {};
...
$(document).ready(function() {
    ...
    function runOpenRouterCheck() {
        ...
        $.get('{{ route('settings.check-openrouter') }}')
            .done(function(data) {
